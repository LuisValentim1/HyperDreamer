...
def decimate_mesh(verts, faces, target, backend='pymeshlab', remesh=False,
    ...
    if backend == 'pyfqmr':
        ...
        solver.simplify_mesh(target_count=target, preserve_border=False,
                             verbose=False)
...
def clean_mesh(verts, faces, v_pct=1, min_f=8, min_d=5, repair=True,
               remesh=True, remesh_size=0.01):
    # verts: [N, 3]
    # faces: [N, 3]
